...
function BuyCreditsModal({onClose,highlight}:{onClose:()=>void;highlight?:string}){
  const{addCredits,account}=useSite();
  const[bought,setBought]=useState<string|null>(null);
  const buy=(pack:typeof CREDIT_PACKS[0])=>{
    // Demo mode: instantly add credits. In production → Stripe Checkout
    addCredits(pack.credits);
    setBought(pack.id);
  };
  return(
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{background:"rgba(0,0,0,0.6)"}} onClick={onClose}>
      <div className="w-full max-w-xl rounded-3xl border border-gray-200 bg-white p-6 shadow-2xl dark:border-gray-800 dark:bg-gray-900" onClick={e=>e.stopPropagation()}>
        <div className="flex items-center justify-between mb-1">
          <div>
            <p className="text-xs font-semibold uppercase tracking-widest text-violet-600 dark:text-violet-400">Buy Credits</p>
            <h2 className="text-xl font-bold text-gray-900 dark:text-white">Top up your AI uses</h2>
          </div>
          <button onClick={onClose} className="rounded-xl p-2 hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-400"><X size={18}/></button>
        </div>
        <p className="text-sm text-gray-500 dark:text-gray-400 mb-5">Credits never expire. Each AI generation uses 1 credit. Free daily uses still reset every day.</p>
        <div className="grid gap-3">
          {CREDIT_PACKS.map(pack=>(
            <div key={pack.id} className={cn("relative rounded-2xl border-2 p-4 transition-all",pack.popular?"border-violet-300 dark:border-violet-700":"border-gray-200 dark:border-gray-700",highlight===pack.id&&"ring-2 ring-violet-400")}>
              {pack.badge&&<span className="absolute -top-2.5 left-4 rounded-full bg-violet-600 px-3 py-0.5 text-[10px] font-bold text-white">{pack.badge}</span>}
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className={cn("flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br text-white text-sm font-bold",pack.color)}>
                    {pack.credits>=1000?"1K":pack.credits}
                  </div>
                  <div>
                    <p className="font-semibold text-gray-900 dark:text-white">{pack.name} — {pack.credits.toLocaleString()} credits</p>
                    <p className="text-xs text-gray-400">{pack.desc} · ~${pack.perUse}/use</p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <span className="text-lg font-bold text-gray-900 dark:text-white">${pack.price}</span>
                  {bought===pack.id
                    ?<span className="flex items-center gap-1 rounded-xl bg-emerald-600 px-4 py-2 text-sm font-bold text-white"><Check size={14}/> Added!</span>
                    :<button onClick={()=>buy(pack)} className={cn("rounded-xl px-4 py-2 text-sm font-bold text-white",pack.popular?"bg-violet-600 hover:bg-violet-700":"bg-gray-800 hover:bg-gray-700 dark:bg-gray-600")}>
                        Buy now
                      </button>
                  }
                </div>
        ...
        <div className="mt-5 rounded-2xl bg-gray-50 p-3 dark:bg-gray-800">
          <p className="text-xs text-gray-500 dark:text-gray-400 text-center">
            🔒 Secure checkout · Credits added instantly · Never expire
            {account.tier==="guest"&&<> · <Link to="/auth/signup" className="text-violet-600 font-semibold" onClick={onClose}>Create a free account</Link> for 25 free uses/day</>}
          </p>
        </div>
        <p className="mt-3 text-center text-[10px] text-gray-400">Demo mode — click Buy now to instantly add credits and see the full experience. Stripe payment processing will be enabled at launch.</p>
      </div>
    </div>
  );
}
      ...
      <form onSubmit={run} className="space-y-4 rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-gray-900">
        ...
        {isLimitReached?(
          <div className="space-y-3">
            ...
            {account.tier==="guest"?(
              <Link to="/auth/signup" className="flex w-full items-center justify-center gap-1.5 rounded-xl border border-gray-200 py-2.5 text-sm font-semibold text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-200">
                <UserPlus size={14}/> Create free account — 25 free uses/day
              </Link>
            ):(
              <Link to="/pricing" className="flex w-full items-center justify-center gap-1.5 rounded-xl border border-gray-200 py-2.5 text-sm font-semibold text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-200">
                <ArrowRight size={14}/> Unlimited plan — $19/month
              </Link>
            )}
          </div>
        ):(
...
function HomePage(){
  ...
  return(
    <>
      <SeoHead title={`${siteName} — Creator + Designer + SEO Toolkit`} description="All-in-one creator toolkit for YouTube, TikTok, Instagram, SEO, logo generation, and AI image creation." schema={homeSchema}/>
...
function CreditStorePage(){
  const{credits,addCredits,account}=useSite();
  const[bought,setBought]=useState<string|null>(null);
  const buy=(pack:typeof CREDIT_PACKS[0])=>{
    addCredits(pack.credits);
    setBought(pack.id);
  ...
  const creditFaqs=[
    {q:"What are credits?",a:"Each AI generation on Logoviking uses 1 credit. Credits are separate from your daily free limit — think of them as a top-up you can use any time."},
    {q:"Do credits expire?",a:"Never. Purchased credits roll over indefinitely and are yours to use whenever you need them."},
    {q:"What happens to my free daily limit?",a:"Your free daily uses (5 for guests, 25 for free accounts) still reset every midnight. Credits are only used after your free limit is exhausted."},
    {q:"Can I use credits with a free account?",a:"Yes. Credits work on any tier — guest, free, or premium."},
    {q:"How do I pay?",a:"Secure checkout via Stripe. Visa, Mastercard, Apple Pay, and Google Pay accepted. Credits are added to your account instantly."},
    {q:"Can I get a refund?",a:"Yes. Unused credits are refundable within 30 days of purchase. Contact us at support@logoviking.com."},
  ];
  return(
    <>
      <SeoHead title="Buy AI Credits" description="Top up your Logoviking AI credits. Credits never expire and let you generate more content any time you run out of daily free uses."/>
      <PageWrap>
        <Breadcrumb items={[{label:"Home",href:"/"},{label:"Pricing",href:"/pricing"},{label:"Buy Credits"}]}/>

        {/* Hero */}
        <div className="mb-10 text-center">
          <div className="inline-flex items-center gap-2 rounded-full border border-violet-200 bg-violet-50 px-4 py-1.5 text-sm font-semibold text-violet-700 dark:border-violet-800 dark:bg-violet-950/30 dark:text-violet-300 mb-4">
            <Zap size={14}/> No subscription needed
          </div>
          <h1 className="text-3xl font-bold tracking-tight text-gray-900 dark:text-white sm:text-4xl">Buy credits. Generate more.</h1>
          <p className="mt-3 max-w-xl mx-auto text-gray-500 dark:text-gray-400">
            Run out of free daily uses? Buy a credit pack and keep generating instantly. Credits never expire and work on every AI tool.
          </p>
          {credits>0&&(
            <div className="mt-4 inline-flex items-center gap-2 rounded-2xl border border-emerald-200 bg-emerald-50 px-5 py-2.5 dark:border-emerald-800 dark:bg-emerald-950/30">
              <Check size={16} className="text-emerald-600"/>
              <span className="font-semibold text-emerald-800 dark:text-emerald-200">You have <strong>{credits.toLocaleString()}</strong> credits remaining</span>
            </div>
          )}
        </div>
        ...
        <div className="grid gap-5 lg:grid-cols-3 max-w-4xl mx-auto">
          {CREDIT_PACKS.map(pack=>(
            <div key={pack.id} className={cn("relative rounded-3xl border-2 p-6 flex flex-col",pack.popular?"border-violet-300 shadow-xl shadow-violet-500/10 dark:border-violet-700":"border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900")}>
              {pack.badge&&<div className="absolute -top-3.5 left-1/2 -translate-x-1/2"><span className="rounded-full bg-violet-600 px-4 py-1 text-xs font-bold text-white">{pack.badge}</span></div>}
              {pack.popular&&<div className="absolute inset-0 rounded-3xl bg-violet-50/40 dark:bg-violet-950/10 -z-10"/>}
              <div className={cn("flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-br text-white text-lg font-bold mb-4",pack.color)}>
                {pack.credits>=1000?"1K":pack.credits}
              </div>
              <p className="text-xs font-bold uppercase tracking-widest text-gray-400 mb-1">{pack.name} Pack</p>
              <div className="flex items-end gap-1 mb-1">
                <span className="text-4xl font-bold text-gray-900 dark:text-white">${pack.price}</span>
                <span className="pb-1 text-sm text-gray-400">one time</span>
              </div>
              <p className="text-2xl font-bold text-violet-600 dark:text-violet-400 mb-1">{pack.credits.toLocaleString()} credits</p>
              <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">{pack.desc}</p>
              <ul className="space-y-2 mb-6 flex-1">
                {[
                  `${pack.credits} AI generations`,
                  "Works on all 8 AI tools",
                  "Never expires",
                  `~$${pack.perUse}/generation`,
                  "Instant delivery",
                ].map(f=>(
                  <li key={f} className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
                    <Check size={14} className="shrink-0 text-emerald-500"/>{f}
                  </li>
                ))}
              </ul>
              {bought===pack.id
                ?<div className="flex items-center justify-center gap-2 rounded-2xl bg-emerald-600 py-3 text-sm font-bold text-white">
                    <Check size={16}/> {pack.credits} credits added!
                  </div>
                :<button onClick={()=>buy(pack)} className={cn("flex w-full items-center justify-center gap-2 rounded-2xl py-3 text-sm font-bold transition-all",pack.popular?"bg-violet-600 text-white shadow-lg shadow-violet-500/25 hover:bg-violet-700":"bg-gray-900 text-white hover:bg-gray-800 dark:bg-gray-700 dark:hover:bg-gray-600")}>
                    Buy now — ${pack.price} <ArrowRight size={14}/>
                  </button>
              }
            </div>
          ))}
        ...
        {/* Trust signals */}
        <div className="mt-8 max-w-4xl mx-auto grid gap-3 sm:grid-cols-3">
          {[
            {icon:Check,label:"Instant delivery",sub:"Credits added the moment payment clears"},
            {icon:Sparkles,label:"Never expire",sub:"Use your credits whenever you need them"},
            {icon:Zap,label:"30-day refund",sub:"Full refund on unused credits, no questions"},
          ].map(({icon:Icon,label,sub})=>(
            <div key={label} className="flex items-start gap-3 rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-900">
              <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl bg-violet-100 dark:bg-violet-950/40">
        ...
        {/* Vs subscription banner */}
        <div className="mt-8 max-w-4xl mx-auto rounded-2xl border border-violet-200 bg-violet-50/60 p-5 dark:border-violet-800 dark:bg-violet-950/10">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="font-semibold text-gray-900 dark:text-white">Need unlimited access every month?</p>
              <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Creator Pro gives you unlimited generations, no ads, and saved projects for $19/month.</p>
            </div>
            <Link to="/pricing" className="shrink-0 flex items-center gap-2 rounded-xl bg-violet-600 px-5 py-2.5 text-sm font-bold text-white hover:bg-violet-700">
        ...
        <div className="mt-10 max-w-4xl mx-auto">
          <SectionTitle title="Credits FAQ" center/>
          <div className="mt-5 grid gap-3 sm:grid-cols-2">
            {creditFaqs.map(({q,a})=>(
              <details key={q} className="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900">
                <summary className="cursor-pointer px-4 py-3.5 text-sm font-semibold text-gray-900 dark:text-white">{q}</summary>
                <p className="px-4 pb-4 text-sm text-gray-500 dark:text-gray-400">{a}</p>
              </details>
            ))}
          </div>
        </div>

        <p className="mt-6 text-center text-xs text-gray-400">Demo mode: clicking "Buy now" adds credits instantly so you can test the full experience. Stripe payment will be live at launch.</p>
      </PageWrap>
    </>
  );
}
